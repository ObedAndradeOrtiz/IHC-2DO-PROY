...
void buscar(String ubicacion) async{
Position position = await _determinePosition();
LatLng origen= new LatLng(position.altitude,position.longitude);
LatLng destino=new LatLng(position.altitude,position.longitude);
Future<void> _getDirections() async {
  DirectionsService directionsService = DirectionsService();
dir.GoogleMapsDirections directions = dir.GoogleMapsDirections(apiKey: "YOUR_API_KEY");

LatLng origin = LatLng(37.7749, -122.4194); // San Francisco, CA
LatLng destination = LatLng(37.3352, -121.8811); // San Jose, CA
...
directionsService.route(
  DirectionsRequest(
    ...
    if (status == DirectionsStatus.ok) {
      // Use the result to display the route on the map
    } else {
      // Handle the error
    }
  },
 ...
 void reproducir(String texto) async {
    ...
    if (Localizacion(texto)) {
     ...
     buscar(response.results[1].name);
    }
  }
